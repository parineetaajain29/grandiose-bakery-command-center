  ...
  return (
    <div className="flex flex-col gap-3">
      {divisions.map((division) => {
        ...
        return (
          <details key={division} className="rounded-card border border-border-subtle bg-bg-panel p-5 shadow-card sm:p-7" open={rows.length <= 6}>
              ...
              <table className="w-full min-w-[560px] border-collapse text-left">
                <thead>
                  <tr className="border-b border-border-subtle font-sans text-xs font-medium text-text-secondary">
                    ...
                    <th className="py-2 pr-4 text-right font-medium">Units sold</th>
                    <th className="py-2 pr-4 text-right font-medium">Sales value</th>
                    <th className="py-2 pr-4 text-right font-medium">Contr. %</th>
                    <th className="py-2 pr-4 text-right font-medium">Rank</th>
                    {isSeasonal && <th className="py-2 pr-4 font-medium">Availability</th>}
                  </tr>
                </thead>
                <tbody className="font-sans text-sm">
                  {rows.map((row) => (
                    ...
                    >
                      ...
                      <td className="py-2 pr-4 text-right font-mono font-tabular text-text-secondary">{row.units.toLocaleString('en-AE')}</td>
                      <td className="py-2 pr-4 text-right font-mono font-tabular text-text-secondary">{formatSkuAed(row.salesAed)}</td>
                      <td className="py-2 pr-4 text-right font-mono font-tabular text-text-secondary">{row.contributionPct.toFixed(1)}%</td>
                      <td className="py-2 pr-4 text-right font-mono font-tabular text-text-secondary">#{row.rank}</td>
