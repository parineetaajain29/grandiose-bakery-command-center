    ...
    <div className="flex flex-col gap-6">
      {employeeState.status === 'ready' && (
        <section className="rounded-card border border-border-subtle bg-bg-panel p-5 shadow-card sm:p-7">
          <p className="font-sans text-xs font-medium text-text-tertiary">Profile</p>
          <div className="mt-1.5 flex flex-wrap items-center justify-between gap-3">
            <h2 className="font-sans text-xl font-semibold text-text-primary">{employeeState.data.name}</h2>
            <span className={`rounded-full border px-2.5 py-0.5 font-sans text-[11px] font-medium ${employeeState.data.active ? 'border-accent-green/40 text-accent-green' : 'border-accent-red/40 text-accent-red'}`}>
              {employeeState.data.active ? 'Active' : 'Deactivated'}
            </span>
          </div>
          <dl className="mt-4 grid grid-cols-2 gap-x-6 gap-y-2 font-sans text-sm sm:grid-cols-4">
            <dt className="text-text-tertiary">ID</dt>
            <dd className="text-text-primary">{employeeState.data.id}</dd>
            <dt className="text-text-tertiary">Department</dt>
            ...
            <dt className="text-text-tertiary">Shift</dt>
          ...
          {isHrAdmin && (
            <div className="mt-4 flex flex-wrap items-center gap-3 border-t border-border-subtle pt-4">
              <button type="button" onClick={handleResetPin} disabled={adminBusy} className="rounded-full border border-border-subtle px-3 py-1.5 font-sans text-xs font-medium text-text-secondary hover:text-text-primary disabled:opacity-50">
                Reset PIN
              </button>
              <button
                type="button"
                ...
                disabled={adminBusy}
                className="rounded-full border border-border-subtle px-3 py-1.5 font-sans text-xs font-medium text-text-secondary hover:text-text-primary disabled:opacity-50"
              >
                {employeeState.data.active ? 'Deactivate' : 'Activate'}
              </button>
              {adminMessage && <p className="font-sans text-xs font-medium text-accent-green">{adminMessage}</p>}
            </div>
          )}
        </section>
      )}
      ...
      {metricsState.status === 'ready' && (
        <>
          ...
          <section className="rounded-card border border-border-subtle bg-bg-panel p-5 shadow-card sm:p-7">
            <p className="font-sans text-xs font-medium text-text-tertiary">Time Allocation</p>
            <div className="mt-3">
      ...
      <section className="rounded-card border border-border-subtle bg-bg-panel p-5 shadow-card sm:p-7">
        ...
        {goalsState.status === 'ready' && goalsState.data.length === 0 && <p className="mt-3 font-sans text-sm text-text-secondary">No goals set.</p>}
        <div className="mt-3 flex flex-col gap-2">
          {goalsState.status === 'ready' &&
            goalsState.data.map((g) => (
              <div key={g.id} className="rounded-card border border-border-subtle p-3">
      ...
      <section className="rounded-card border border-border-subtle bg-bg-panel p-5 shadow-card sm:p-7">
        <p className="font-sans text-xs font-medium text-text-tertiary">Feedback History</p>
        {feedbackState.status === 'ready' && feedbackState.data.length === 0 && <p className="mt-3 font-sans text-sm text-text-secondary">No feedback yet.</p>}
        <div className="mt-3 flex flex-col gap-2">
  ...
  return (
    <section className="rounded-card border border-border-subtle bg-bg-panel p-5 shadow-card sm:p-7">
      <div className="flex flex-wrap gap-2">
        <button type="button" onClick={() => setMode(mode === 'goal' ? 'none' : 'goal')} className="rounded-full border border-accent-blue px-3.5 py-1.5 font-sans text-xs font-medium text-accent-blue hover:bg-accent-blue hover:text-[#04070d]">
          Set Goal
        </button>
        <button type="button" onClick={() => setMode(mode === 'feedback' ? 'none' : 'feedback')} className="rounded-full border border-accent-blue px-3.5 py-1.5 font-sans text-xs font-medium text-accent-blue hover:bg-accent-blue hover:text-[#04070d]">
          Add Feedback
        </button>
      </div>
...
function GoalForm({ employeeId, onSaved }: { employeeId: string; onSaved: () => void }) {
  ...
  const inputClass = 'mt-1 w-full rounded-lg border border-border-subtle bg-bg-primary px-3 py-2 font-sans text-sm text-text-primary focus:border-accent-blue/60 focus:outline-none';
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-4 grid grid-cols-1 gap-3 border-t border-border-subtle pt-4 sm:grid-cols-2 lg:grid-cols-3">
      ...
      <div className="flex items-end">
        <button type="submit" disabled={busy} className="rounded-full border border-accent-blue bg-accent-blue px-4 py-2 font-sans text-xs font-medium text-[#04070d] disabled:opacity-50">
