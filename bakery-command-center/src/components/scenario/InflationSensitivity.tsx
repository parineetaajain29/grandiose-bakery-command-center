...
function MetricCard({ eyebrow, value, caption, tone = 'neutral' }: { eyebrow: string; value: string; caption?: string; tone?: 'green' | 'red' | 'neutral' }) {
  return (
    <div className="rounded-card border border-border-subtle bg-bg-panel p-4">
      ...
      {caption && <p className={`mt-1 font-sans text-xs ${TONE_CLASS[tone]}`}>{caption}</p>}
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col gap-6">
      <section className="rounded-card border border-border-subtle bg-bg-panel p-5 shadow-card sm:p-7">
          ...
          <div className="flex flex-col items-center">
            <div className="relative h-48 w-48">
              ...
              <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
                ...
                <p className="font-sans text-xs text-text-tertiary">net / unit</p>
              </div>
            </div>
          </div>
        ...
        <p className="mt-4 font-sans text-xs text-text-tertiary">{inflationSensitivity.context}</p>
      </section>
    </div>
  );
}
